...
xx <- county_train_stayhome %>% 
  distinct(fips, state, county, nchs, pop, stayhome)
...
safegr %>% 
  group_by(fips) %>% 
  summarise(max_relative = max(relative_week)) %>% 
  ungroup() %>% 
  arrange(max_relative)
```

```{r}
safegr$rebound_40 <- safegr$date
safegr$rebound_40[safegr$roll_relative_week > 0.6] <- as.Date("2019-12-31")
...
summary(safegr$rebound_40)
summary(safegr$days_btwn_50_40)
```

```{r}
zz %>% 
  left_join(safegr) %>% 
  distinct(fips, state, county, nchs, rebound_40) %>% 
  arrange(rebound_40)
```
...
```{r}
zz %>% 
  left_join(distinct(safegr, fips, days_btwn_50_40)) %>%
  ggplot() +
