...
parameters {
  ...
  real<lower=0.02, upper=30.0> overdisp;
...
transformed parameters {
  ...
  vector[N] hockey_stick = 0.01 * clamp_zero(days_since_intrv - lag);
  //                        ^ 0.01 is just for stability
  matrix[N, 2] tpoly_post = append_col(hockey_stick, square(hockey_stick));
...
generated quantities {
  ...
  for (n in 1:N) {
    ...
    real phi_ = fmin(fmax(overdisp, 0.01), 30.0);
