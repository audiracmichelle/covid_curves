...
```{r}
fit = read_rds("./model_full_rstan_var_revised_cable_bent.rds")
```
...
```{r}
county_fit_posterior = my_posterior_predict_cable_bent(fit, evaldata, type="decrease")
county_fit = county_fit_posterior$yhat_no_rand_eff %>% 
  melt() %>% 
...
county_fit_posterior_up = my_posterior_predict_cable_bent(fit, evaldata_up, type="decrease")
county_fit_up = county_fit_posterior_up$yhat_no_rand_eff %>% 
  melt() %>% 
  `names<-`(c("sim", "row", "yhat")) %>% 
  left_join(
    select(
...
county_fit_posterior_down = my_posterior_predict_cable_bent(fit, evaldata_down, type="decrease")
county_fit_down = county_fit_posterior_down$yhat_no_rand_eff %>% 
  melt() %>% 
  `names<-`(c("sim", "row", "yhat")) %>% 
  left_join(
    select(
